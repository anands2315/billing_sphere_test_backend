const DebitNoteVoucher = require("../models/debit_note_voucher_model");
...
async function processVendorBill({ bill, parsedAmount, isDebit, session }) {
  ...
  if (bill.type === "Debit Note w/o Stock") {
    if (isDebit) {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
    } else {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
...
async function processCustomerBill({ bill, parsedAmount, isDebit, session }) {
  ...
  } else if (bill.type === "Debit Note w/o Stock") {
    if (isDebit) {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
    } else {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
...
const DebitNoteVoucherController = {
  // create Debit Note Voucher
  createDebitNoteVoucher: async (req, res) => {
    ...
    try {
      ...
      const journalVch = new DebitNoteVoucher(journalData);
  ...
  getAllDebitNote: async (req, res) => {
    try {
      const creditNote = await DebitNoteVoucher.find({});
  ...
  // Fetch Journal By Id
  getDebitNoteById: async (req, res) => {
    try {
      const { id } = req.params;

      const creditNote = await DebitNoteVoucher.findById(id);
      if (!creditNote) {
        return res
          .status(404)
  ...
  deleteDebitNoteVoucher: async (req, res) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const debitNoteVoucher = await DebitNoteVoucher.findById(id).session(
        session
      );
      if (!debitNoteVoucher) {
        throw new Error("Debit Note Voucher not found.");
      }
      ...
      await DebitNoteVoucher.findByIdAndDelete(id, { session });
      ...
      res.status(200).json({
        success: true,
        message: "Debit Note Voucher deleted",
      });
    } catch (error) {
  ...
  updateDebitNoteVoucher: async (req, res) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const existingVch = await DebitNoteVoucher.findById(id).session(session);
      if (!existingVch) {
        throw new Error("Debit Note Voucher not found.");
      }
...
module.exports = DebitNoteVoucherController;
