const GstExpenseVoucher = require("../models/gst_expense_voucher_model");
...
async function processVendorBill({ bill, parsedAmount, isDebit, session }) {
  ...
  if (bill.type === "Gst Expense w/o Stock") {
    if (isDebit) {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
    } else {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
...
async function processCustomerBill({ bill, parsedAmount, isDebit, session }) {
  ...
  } else if (bill.type === "Gst Expense w/o Stock") {
    if (isDebit) {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
    } else {
      bill.dueAmount = parseFloat(bill.dueAmount) - parsedAmount;
...
const GstExpenseVoucherController = {
  // create Gst Expense Voucher
  createGstExpenseVoucher: async (req, res) => {
    ...
    try {
      ...
      const journalVch = new GstExpenseVoucher(journalData);
      ...
      return res.status(201).json({
        success: true,
        message: "Gst Expense voucher created successfully!",
        data: journalVch,
      });
    } catch (error) {
  ...
  getAllGstExpense: async (req, res) => {
    try {
      const creditNote = await GstExpenseVoucher.find({});
  ...
  // Fetch Journal By Id
  getGstExpenseById: async (req, res) => {
    try {
      const { id } = req.params;

      const creditNote = await GstExpenseVoucher.findById(id);
      if (!creditNote) {
        return res
          .status(404)
  ...
  deleteGstExpenseVoucher: async (req, res) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const debitNoteVoucher = await GstExpenseVoucher.findById(id).session(
        session
      );
      if (!debitNoteVoucher) {
        throw new Error("Gst Expense Voucher not found.");
      }

      // Step 1: Revert `entries`
      for (const entry of debitNoteVoucher.entries) {
        const { ledger: ledgerId, debit, credit } = entry;
      ...
      for (const bill of debitNoteVoucher.billwise) {
        const {
          ledger: ledgerId,
          billType,
          amount,
      ...
      await GstExpenseVoucher.findByIdAndDelete(id, { session });
      ...
      res.status(200).json({
        success: true,
        message: "Gst Expense Voucher deleted",
      });
    } catch (error) {
  ...
  updateGstExpenseVoucher: async (req, res) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const existingVch = await GstExpenseVoucher.findById(id).session(session);
      if (!existingVch) {
        throw new Error("Gst Expense Voucher not found.");
      }
      ...
      return res.status(200).json({
        success: true,
        message: "Debite Note voucher updated successfully!",
        data: existingVch,
      });
    } catch (error) {
      await session.abortTransaction();
...
module.exports = GstExpenseVoucherController;
